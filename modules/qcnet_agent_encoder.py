...
class QCNetAgentEncoder(nn.Module):
    ...
    def forward(self,
        ...
        if self.dataset == 'argoverse_v2':
            ...
            vel = data['agent']['velocity'][:, :self.num_historical_steps, :self.input_dim].contiguous()
            length = data['agent']['length'][:, :self.num_historical_steps].contiguous()
            width = data['agent']['width'][:, :self.num_historical_steps].contiguous()
            height = data['agent']['height'][:, :self.num_historical_steps].contiguous()
            categorical_embs = [
                self.type_a_emb(data['agent']['type'].long()).repeat_interleave(repeats=self.num_historical_steps,
                                                                                dim=0),
            ]
